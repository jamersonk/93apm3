...
Consider the propositional function $P(n)$. Mathematical induction involves proving $P(n)$ is true by: (1) proving that $P(1)$, the \textit{basis of induction}, is true; (2) proving that for all positive integers $k$, if $P(k)$ is true, that $P(k+1)$ is true ($P(k) \rightarrow P(k+1)$) This is the \textit{induction hypothesis}.
\begin{remark}
Proof by Induction does not show that $P(k)$ is true. It only shows that if we assume that $P(k)$ is true, that $P(k+1)$ is also true.
\end{remark}
\end{document}
