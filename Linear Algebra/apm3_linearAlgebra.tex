...
\begin{itemize}
	\item $v+w$,
	\item $cw$, and
	\item any such linear combination (i.e. $cv+dw$).
\end{itemize}
...
\end{itemize}
\end{exercise}
\subsubsection{Columnspace \& Nullspace}
\begin{definition}
The \textbf{columnspace} $C\left(A\right)$ is the subspace of $\mathbb{R}^n$ which contains all the linear combinations of the columns of $A$. 
\end{definition}
\begin{remark}
When the number of equations $m$ $>$ the number of unknowns $n$, then $A\textbf{x}=b$ does not always have a solution. $Ax=b$ only has a subspace when $b$ is in the column space of $A$.
\end{remark}
\begin{definition}
The \textbf{nullspace} $N\left(A\right)$ is the subspace of $\mathbb{R}^n$ which contains all the solutions to the $\textbf{x}$ such that $A\textbf{x}=0$.
\end{definition}
